...
class G1Example : public rclcpp::Node {
 ...
 public:
  ...
  //   const std::shared_ptr<const MotorState> ms =
  ...
  //             static_cast<float>((q_des - ms->q.at(i)) * ratio +
  //             ms->q.at(i));
  ...
  //             arm_cmd_mutex_);  // 增加一个简单的互斥锁
  //         for (int i = 0; i < 7; ++i) {
  //           // 强烈建议在这里加上限位，例如 clamp(val, -3.14, 3.14)
  //           right_arm_target_q_[i] = static_cast<float>(msg->position[i]);
  //         }
  //         has_received_arm_cmd_ = true;
  //       }
